...
out=zeros(1,n);
for ii=1:n
     if angle(rx(ii))>=-pi/8 && angle(rx(ii))<=pi/8
        out(ii)=1;
    elseif angle(rx(ii))>=pi/8 && angle(rx(ii))<=3*pi/8
        out(ii)=1/(sqrt(2))+1i/(sqrt(2));
    elseif angle(rx(ii))>=3*pi/8 && angle(rx(ii))<=5*pi/8
        out(ii)=1i;
    elseif angle(rx(ii))>=5*pi/8 && angle(rx(ii))<=7*pi/8
        out(ii)=-1/(sqrt(2))+1i/(sqrt(2));
    elseif angle(rx(ii))>=7*pi/8 && angle(rx(ii))<=-7*pi/8
        out(ii)=-1i;
    elseif angle(rx(ii))>=-7*pi/8 && angle(rx(ii))<=-5*pi/8
        out(ii)=-1/(sqrt(2))-1i/(sqrt(2));
    elseif angle(rx(ii))>=-5*pi/8 && angle(rx(ii))<=-3*pi/8
        out(ii)=-1i;
     else
        out(ii)=1/(sqrt(2))-1i/(sqrt(2));
     end
